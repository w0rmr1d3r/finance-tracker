...
def client():
    ...
    with patch("finance_tracker.__main__._save_entries"):
        from finance_tracker.__main__ import app  # ruff: ignore[import-outside-top-level]
...
def test_delete_category_persists_updated_json(client):
    """Verify categories.json is written before _save_entries is called."""
    cats = {
    ...
    assert resp.status_code == 200
    assert call_order == ["open", "save"], "categories.json must be written before _save_entries is called"
