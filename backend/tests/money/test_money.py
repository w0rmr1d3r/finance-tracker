...
def test_equal_money_have_same_hash():
    currency_code = Faker().currency_code()
    money_one = Money(currency_code=currency_code, amount=1.0)
    money_two = Money(currency_code=currency_code, amount=1.0)
    assert hash(money_one) == hash(money_two)


def test_money_can_be_used_in_set():
    currency_code = Faker().currency_code()
    money_one = Money(currency_code=currency_code, amount=1.0)
    money_two = Money(currency_code=currency_code, amount=1.0)
    assert len({money_one, money_two}) == 1


def test_different_money_have_different_hash():
    currency_code = Faker().currency_code()
    money_one = Money(currency_code=currency_code, amount=1.0)
    money_two = Money(currency_code=currency_code, amount=2.0)
    assert hash(money_one) != hash(money_two)
