...
def test_entry_reader_reads_an_entry(reader):
    ...
    assert len(result) == 1
    assert result[0].balance == Money(currency_code="EUR", amount=0.0)
    ...
    assert result[0].quantity == Money(currency_code="EUR", amount=1_000)
...
def test_entry_reader_reads_an_entry_new_format(reader):
    current_path = pathlib.Path(__file__).parent.resolve()
    path_to_file = f"{current_path}/files/test_entry_reader_one_entry_new_format.csv"
    result = reader.read_from_file(path_to_file=path_to_file)
...
def test_float_in_str_to_str_returns_correct_value():
    assert 1000.0 == EntryReader.float_in_str_to_str("1000,00")
